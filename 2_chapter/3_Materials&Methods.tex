...
As illustrated in Figure~\ref{fig:DynaDA3_architecture}, the DynaDA3 architecture is built on top of the Depth Anything V3 (DA3) framework~\cite{lin2025da3}, but it does not retrain the DA3 foundation model itself. Instead, the pretrained DA3 module is kept frozen and reused as a geometric prior extractor, while a newly introduced uncertainty-aware DPT head is trained on top of DA3 outputs. For an input batch of multi-frame images $\mathbf{I}\in\mathbb{R}^{B\times N\times 3\times H\times W}$, the frozen DA3 stage first produces dense depth, per-pixel depth confidence, and four intermediate transformer feature maps. These signals are then fused by a trainable \texttt{UncertaintyDPT} head to predict a two-class segmentation logit map, where class 0 denotes certain or static regions and class 1 denotes uncertain or dynamic regions. In this way, DynaDA3 preserves the broad geometric generalization capability of DA3 while specializing only the final uncertainty filtering stage for dynamic surgical environments.
% 如图~\ref{fig:DynaDA3_architecture} 所示，
% DynaDA3 架构建立在 Depth Anything V3 (DA3) 框架之上，
% 但并不会重新训练 DA3 基础模型本体。
% 相反，预训练的 DA3 模块被整体冻结并复用为几何先验提取器，
% 而一个新引入的不确定性感知 DPT 头则建立在 DA3 输出之上进行训练。
% 对于一个多帧输入批次 $\mathbf{I}\in\mathbb{R}^{B\times N\times 3\times H\times W}$，
% 冻结的 DA3 阶段首先产生稠密深度、逐像素深度置信度以及四层中间 Transformer 特征。
% 随后，这些信号会被可训练的 \texttt{UncertaintyDPT} 融合，
% 输出一个两类分割 logit 图，其中类别 0 表示确定或静态区域，
% 类别 1 表示不确定或动态区域。
% 因而，DynaDA3 在保留 DA3 通用几何泛化能力的同时，
% 仅针对动态手术环境中的最终不确定区域过滤阶段进行任务特化。

\subsubsection*{DA3 Backbone and Geometric Outputs}
The frozen DA3 stage follows the original modular design of a DINOv2 backbone, a Dual-DPT dense prediction head, and an optional camera branch~\cite{lin2025da3}. In the implementation used here, the large DA3 variant uses a ViT-L backbone and exports intermediate features from transformer layers $\{11,15,19,23\}$, while the giant variant uses a ViT-G backbone with layers $\{21,27,33,39\}$. During training, \texttt{DynaDA3.forward()} directly calls \texttt{self.da3.model(image, export\_feat\_layers=...)} under \texttt{torch.no\_grad()}, so the DA3 stage remains strictly frozen. The returned dictionary contains four exported feature tensors stored in \texttt{out['aux']} under keys of the form \texttt{feat\_layer\_<idx>}, each with shape $\mathbb{R}^{B\times N\times h\times w\times C}$, a dense depth map \texttt{out['depth']}\,$\in\mathbb{R}^{B\times N\times H\times W}$, and a dense confidence map \texttt{out['depth\_conf']}\,$\in\mathbb{R}^{B\times N\times H\times W}$.
% 冻结的 DA3 阶段延续了原始框架的模块化设计，
% 即由 DINOv2 主干、Dual-DPT 稠密预测头以及可选的相机分支组成。
% 在本文采用的实现中，
% DA3 large 版本使用 ViT-L 主干，并导出 $\{11,15,19,23\}$ 四层 Transformer 特征；
% DA3 giant 版本则使用 ViT-G 主干，并导出 $\{21,27,33,39\}$ 四层特征。
% 在训练时，\texttt{DynaDA3.forward()} 会在 \texttt{torch.no\_grad()} 下
% 直接调用 \texttt{self.da3.model(image, export\_feat\_layers=...)}，
% 因而 DA3 阶段始终保持冻结。
% 返回结果中包含四个导出的中间特征张量、
% 稠密深度图 \texttt{out['depth']} 以及置信度图 \texttt{out['depth\_conf']}。

Before entering the uncertainty head, the wrapper explicitly reshapes these DA3 outputs. For each exported layer $\ell_k$, the feature tensor is converted from
\begin{center}
    $\displaystyle \mathbf{F}^{(\ell_k)} \in \mathbb{R}^{B\times N\times h\times w\times C}$
\end{center}
to
\begin{center}
    $\displaystyle \tilde{\mathbf{F}}^{(\ell_k)} \in \mathbb{R}^{(BN)\times C\times h\times w},$
\end{center}
while the depth and confidence maps are reshaped from $\mathbb{R}^{B\times N\times H\times W}$ to $\mathbb{R}^{(BN)\times 1\times H\times W}$. Thus, the uncertainty head does not operate on temporally aggregated tokens; instead, it processes all sampled frames independently after flattening the batch and frame dimensions into a single dimension $M=BN$. In the inference path, the same information is obtained through \texttt{self.da3.inference(...)} and then read from \texttt{prediction.aux}, \texttt{prediction.depth}, and \texttt{prediction.conf}. Therefore, in both training and inference, the uncertainty head receives the same three signal groups: four DA3 feature maps, one depth map, and one confidence map per frame.
% Dual-DPT 头会先将四层 token 重新组装成不同分辨率的类图像特征图，
% 然后在不确定性头之前，
% 包装器会显式重排这些 DA3 输出的张量形状。
% 对于每个导出层 $\ell_k$，
% 特征会从 $\mathbb{R}^{B\times N\times h\times w\times C}$
% 变换为 $\mathbb{R}^{(BN)\times C\times h\times w}$，
% 深度图和置信度图则从 $\mathbb{R}^{B\times N\times H\times W}$
% 变为 $\mathbb{R}^{(BN)\times 1\times H\times W}$。
% 因而，不确定性头并不直接处理时序聚合后的 token，
% 而是先将 batch 和帧维合并为 $M=BN$，
% 再对每一帧独立进行密集分割。
% 在推理阶段，相同信息则通过 \texttt{self.da3.inference(...)}
% 从 \texttt{prediction.aux}、\texttt{prediction.depth} 和 \texttt{prediction.conf}
% 中读取。

Keeping the complete DA3 stage frozen is a deliberate design choice. The goal of DynaDA3 is not to relearn generic geometry from limited task-specific data, but to reuse DA3 as a stable geometric foundation and to add a specialized uncertainty segmentation layer on top. This strategy reduces the number of trainable parameters substantially and helps preserve the robustness of the pretrained model in out-of-domain scenes.
% 将完整的 DA3 阶段保持冻结是一个有意的设计选择。
% DynaDA3 的目标并不是在有限的任务数据上重新学习通用几何，
% 而是复用 DA3 作为稳定的几何基础，
% 再在其之上叠加一个专门的不确定区域分割层。
% 这一策略显著减少了需要训练的参数量，
% 同时也有助于保留预训练模型在跨域场景中的鲁棒性。

\subsubsection*{Uncertainty-Aware DPT Head}
Although named \texttt{UncertaintyDPT}, the current implementation is a lightweight convolutional fusion head operating on DA3 features rather than a full multi-stage decoder. Let $M=BN$ denote the flattened frame dimension, let the legal input resolution satisfy $H=ph$ and $W=pw$ with patch size $p=14$, and let the embedding dimension be $C_e=2048$. The four DA3 feature maps are first projected independently by $1\times 1$ convolutions:
\begin{center}
    $\displaystyle
    \mathbf{P}_k = \phi_k\!\left(\tilde{\mathbf{F}}^{(\ell_k)}\right)
    = \operatorname{ReLU}\!\left(\operatorname{Conv}^{1\times 1}_k\!\left(\tilde{\mathbf{F}}^{(\ell_k)}\right)\right),
    \qquad
    \mathbf{P}_k \in \mathbb{R}^{M\times C_e\times h\times w},
    $
\end{center}
and then summed:
\begin{center}
    $\displaystyle
    \mathbf{P}_{img} = \sum_{k=1}^{4}\mathbf{P}_k.
    $
\end{center}
This part corresponds exactly to the four \texttt{projects\_layer} modules in the code.
% DynaDA3 中唯一可训练的部分是 \texttt{UncertaintyDPT} 头，
% 它负责对不确定或动态区域进行二值分割。
% 尽管命名为 \texttt{UncertaintyDPT}，
% 当前实现实际上是一个建立在 DA3 特征之上的轻量级卷积融合头，
% 而不是完整的多阶段 DPT 解码器。
% 记展平后的帧维为 $M=BN$，
% 合法输入分辨率满足 $H=ph$、$W=pw$，
% 其中 patch 大小 $p=14$，嵌入维度 $C_e=2048$。
% 四层 DA3 特征首先分别经过 $1\times 1$ 卷积和 ReLU 投影，
% 然后相加得到图像特征分支 $\mathbf{P}_{img}$。

The geometric branch is constructed explicitly from the DA3 depth and confidence outputs. First, both maps are normalized frame-wise by robust percentile normalization. For a scalar map $\mathbf{X}\in\mathbb{R}^{M\times 1\times H\times W}$, the code computes
\begin{center}
    $\displaystyle
    q_{low}^{(m)} = Q_{0.05}\!\left(\mathbf{X}^{(m)}\right), \qquad
    q_{high}^{(m)} = Q_{0.95}\!\left(\mathbf{X}^{(m)}\right),
    $
\end{center}
and then
\begin{center}
    ...
    \operatorname{clip}
    \left(
    \frac{\mathbf{X}^{(m)} - q_{low}^{(m)}}
         {\max\!\left(q_{high}^{(m)} - q_{low}^{(m)}, \varepsilon\right)},
    0, 1
    \right),
    \qquad \varepsilon = 10^{-6},
    $
\end{center}
where $Q_{0.05}$ and $Q_{0.95}$ denote the 5th and 95th percentiles, respectively. Applying this operation to depth and confidence yields normalized maps $\bar{\mathbf{D}}$ and $\bar{\mathbf{C}}$. This design is directly taken from the helper function \texttt{\_geo\_normalize()} and prevents extreme outliers from dominating the feature scale.
% 当前版本任务头最有辨识度的结构设计在于：
% 几何分支则由 DA3 输出的深度图和置信度图显式构造。
% 代码首先对每一帧分别执行稳健的百分位归一化：
% 取 5\% 和 95\% 分位数作为有效范围，
% 再将结果裁剪到 $[0,1]$。
% 该设计直接对应 \texttt{\_geo\_normalize()}，
% 其目的是避免极端值主导几何特征尺度。

Next, a depth-gradient map is computed by first-order forward differences:
\begin{center}
    $\displaystyle
    \mathbf{G}_x(u,v) = \left|\bar{\mathbf{D}}(u,v+1)-\bar{\mathbf{D}}(u,v)\right|,
    \qquad
    \mathbf{G}_y(u,v) = \left|\bar{\mathbf{D}}(u+1,v)-\bar{\mathbf{D}}(u,v)\right|,
    $
\end{center}
and the final gradient prior is formed as
\begin{center}
    $\displaystyle
    \mathbf{G} = \operatorname{Pad}_x(\mathbf{G}_x) + \operatorname{Pad}_y(\mathbf{G}_y),
    $
\end{center}
where zero padding restores the original spatial size along the last column and last row. In code, the geometric priors are then transferred from image space to patch space by \texttt{pixel\_unshuffle} with factor $p=14$. For a generic tensor $\mathbf{X}\in\mathbb{R}^{M\times C\times H\times W}$, this operation can be written as
\begin{center}
    $\displaystyle
    \operatorname{PU}(\mathbf{X})_{m,\;cp^2+ip+j,\;a,\;b}
    =
    \mathbf{X}_{m,\;c,\;pa+i,\;pb+j},
    $
\end{center}
with $i,j\in\{0,\dots,p-1\}$, $a\in\{0,\dots,h-1\}$, and $b\in\{0,\dots,w-1\}$. Thus, each $14\times 14$ local patch is folded into the channel dimension.
% 随后，代码通过一阶前向差分计算深度梯度先验，
% 并通过在最后一列与最后一行补零的方式恢复原始空间尺寸。
% 接着，所有几何图都会经由因子为 $p=14$ 的 \texttt{pixel\_unshuffle}
% 从图像空间映射到 patch 空间，
% 即把每个 $14\times14$ patch 的局部结构折叠进通道维度。

Defining
\begin{center}
    $\displaystyle
    \tilde{\mathbf{D}}=\operatorname{PU}(\bar{\mathbf{D}}),\qquad
    \tilde{\mathbf{C}}=\operatorname{PU}(\bar{\mathbf{C}}),\qquad
    \tilde{\mathbf{G}}=\operatorname{PU}(\mathbf{G}),
    $
\end{center}
the code constructs two additional heuristic priors in patch space:
\begin{center}
    $\displaystyle
    \tilde{\mathbf{U}} = (1-\tilde{\mathbf{C}})\odot(1-\tilde{\mathbf{D}}), \qquad
    \tilde{\mathbf{L}} = 1-\tilde{\mathbf{C}}.
    $
\end{center}
The full geometric input is then
\begin{center}
    $\displaystyle
    \mathbf{X}_{geo}
    =
    ...
    \tilde{\mathbf{L}}
    \right]
    \in \mathbb{R}^{M\times 5p^2\times h\times w}.
    $
\end{center}
Because $p=14$, this branch has $5p^2=980$ input channels. The geometric tensor is projected by a $1\times 1$ convolution and ReLU:
\begin{center}
    $\displaystyle
    \mathbf{P}_{geo}
    =
    \operatorname{ReLU}\!\left(\operatorname{Conv}^{1\times1}_{geo}(\mathbf{X}_{geo})\right)
    \in \mathbb{R}^{M\times C_e\times h\times w}.
    $
\end{center}
The image and geometric branches are then fused by channel concatenation followed by a $3\times 3$ convolution, batch normalization, and ReLU:
\begin{center}
    $\displaystyle
    \mathbf{F}_{fuse}
    =
    ...
    \operatorname{BN}
    \left(
    \operatorname{Conv}^{3\times3}
    \left(
    \operatorname{Cat}[\mathbf{P}_{geo},\mathbf{P}_{img}]
    \right)
    \right)
    \right)
    \in \mathbb{R}^{M\times C_e\times h\times w}.
    $
\end{center}
Finally, the output block first reduces the channel dimension from $2048$ to $1024$ with a $3\times 3$ convolution, batch normalization, and ReLU, then predicts $2p^2$ channels by a $1\times 1$ convolution, and finally restores full resolution by \texttt{PixelShuffle}(14):
\begin{center}
    $\displaystyle
    \mathbf{Z}_{patch}\in\mathbb{R}^{M\times 2p^2\times h\times w},
    \qquad
    \mathbf{Z}=\operatorname{PixelShuffle}_{p}(\mathbf{Z}_{patch})
    \in\mathbb{R}^{M\times 2\times H\times W}.
    $
\end{center}
The final hard segmentation mask used at inference time is obtained by
\begin{center}
    $\displaystyle
    \hat{\mathbf{M}}(u,v)=\arg\max_{c\in\{0,1\}} \mathbf{Z}_{c}(u,v).
    $
\end{center}
Compared with earlier internal variants that relied only on summed DINO features or simple confidence gating, this final implementation injects explicit depth- and confidence-derived priors while still keeping the trainable head compact. During training, \texttt{DynaDA3.forward()} returns the logits $\mathbf{Z}$ directly; during inference, \texttt{DynaDA3.inference()} first executes the full DA3 pipeline and then attaches both $\mathbf{Z}$ and the hard mask $\hat{\mathbf{M}}$ to the prediction object.
% 与早期仅依赖 DINO 特征求和或简单 confidence gating 的内部版本相比，
% 与早期仅依赖 DINO 特征求和或简单 confidence gating 的内部版本相比，
% 当前最终实现会显式引入由深度和置信度导出的几何先验，
% 同时又保持任务头本身足够紧凑。
% 训练时 \texttt{DynaDA3.forward()} 直接返回 logits $\mathbf{Z}$；
% 推理时 \texttt{DynaDA3.inference()} 则先完整执行 DA3，
% 再把 logits 和硬掩码一并写回 prediction 对象。

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Training}
Training is restricted to the uncertainty segmentation head. The DA3 backbone, the Dual-DPT geometric head, and the optional camera branch remain frozen throughout optimization, and only the parameters of \texttt{model.uncertainty\_head} are updated. In the training path, the input tensor must satisfy the legal DA3 size constraint $H=14h$ and $W=14w$; this is handled by data loading and cropping before the forward pass. Given an input batch $\mathbf{I}\in\mathbb{R}^{B\times N\times 3\times H\times W}$, the frozen DA3 stage extracts four feature tensors together with depth and confidence maps, after which the wrapper flattens the batch and frame dimensions into $M=BN$. The uncertainty head therefore returns logits
\begin{center}
    $\displaystyle
    \mathbf{Z}\in\mathbb{R}^{M\times 2\times H\times W},
    $
\end{center}
that is, one two-class logit vector for every pixel of every sampled frame.
% 训练阶段仅优化不确定性分割头。
% DA3 主干、Dual-DPT 几何头以及可选相机分支在整个优化过程中始终保持冻结，
% 只有 \texttt{model.uncertainty\_head} 的参数会被更新。
% 这一选择能够保留预训练几何先验，
% 并将学习过程限制在识别不可靠图像区域这一下游任务上。
% 在训练路径中，输入尺寸必须满足 DA3 的合法约束 $H=14h$、$W=14w$；
% 这一点由数据加载和裁剪过程预先保证。
% 给定输入批次 $\mathbf{I}\in\mathbb{R}^{B\times N\times 3\times H\times W}$，
% 冻结的 DA3 会先提取四层特征以及深度图、置信度图，
% 然后包装器将 batch 和帧维展平为 $M=BN$。
% 因而，不确定性头最终返回的 logits 张量为
% $\mathbf{Z}\in\mathbb{R}^{M\times 2\times H\times W}$。

The supervision is formulated as pixel-wise binary classification with label 0 for certain or static pixels and label 1 for uncertain or dynamic pixels. Let $\mathbf{Y}\in\{0,1\}^{M\times H\times W}$ denote the ground-truth mask after the same flattening, and let $\mathbf{Z}_{m,c,u,v}$ be the pre-softmax logit of class $c$ at pixel $(u,v)$ in flattened frame $m$. The training objective is the standard per-pixel cross-entropy loss applied directly to logits:
% 监督形式被建模为逐像素二分类，
% 其中标签 0 表示确定或静态像素，标签 1 表示不确定或动态像素。
% 将同样展平后的真值记为 $\mathbf{Y}\in\{0,1\}^{M\times H\times W}$，
% 将 logits 记为 $\mathbf{Z}_{m,c,u,v}$，
% 则训练目标就是直接作用在 logits 上的逐像素交叉熵损失。
\begin{center}
    $\displaystyle
    \mathcal{L}_{unc}
    =
    ...
    \sum_{v=1}^{W}
    \log
    \frac{\exp\!\left(\mathbf{Z}_{m,\mathbf{Y}_{m,u,v},u,v}\right)}
         {\sum_{c=0}^{1}\exp\!\left(\mathbf{Z}_{m,c,u,v}\right)}.
    $
\end{center}
Equivalently, this can be understood as applying a softmax over the two channels of $\mathbf{Z}$ and penalizing the negative log-likelihood of the ground-truth class at every pixel. The code does not apply a sigmoid or threshold inside the model; instead, it returns raw logits during training and computes the hard mask only at inference time via an \texttt{argmax} over the class dimension.
% 该损失会直接监督不确定性头输出的两通道 logits，
% 与实现中采用的二值掩码预测目标保持一致。
% 等价地，也可以把它理解为先对 logits 的两个通道做 softmax，
% 再对每个像素的真值类别求负对数似然。
% 代码在模型内部不会使用 sigmoid 或阈值化；
% 训练时输出原始 logits，
% 只有推理时才通过 \texttt{argmax} 生成硬掩码。

The training data combines both generic dynamic-scene videos and endoscopic data. For the generic part, we use video sequences derived from the Wild-SLAM Mocap Dataset, the Wild-SLAM iPhone Dataset, and additional public sequences such as the Bonn dataset. For these data, binary uncertainty masks are generated with SAM3-assisted interactive segmentation and then manually corrected to obtain reliable labels. Beyond such general-scene data, we also include endoscopic and arthroscopic videos from datasets such as ArthroPhase, so that the uncertainty head is exposed to surgical appearance changes, instrument motion, fluid reflections, and other domain-specific disturbances. This mixed-data strategy improves the robustness of the uncertainty head without modifying the pretrained DA3 geometry model.
% 训练数据同时结合了通用动态场景视频和内窥镜场景数据。
% 在通用场景部分，
% 本文使用了来自 Wild-SLAM Mocap Dataset、Wild-SLAM iPhone Dataset
% 以及 Bonn 等其他公开数据集的视频序列。
% 对于这类数据，二值不确定性掩码通过基于 SAM3 的交互式分割生成，
% 随后再进行人工修正，以获得可靠标签。
% 除了这类通用场景数据之外，
% 本文还加入了内窥镜和关节镜场景视频，
% 例如 ArthroPhase 一类的数据集，
% 使得不确定性头能够接触到手术场景中特有的外观变化、
% 器械运动、液体反光以及其他领域特定干扰。
% 这种混合数据训练策略在不修改预训练 DA3 几何模型的前提下，
% 提升了不确定性头的整体鲁棒性。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
